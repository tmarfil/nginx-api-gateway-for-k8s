...
Familiarize yourself with the lab environment before we get started.

The lab environment consists of a single K8s cluster (`microk8s <https://microk8s.io/>`_) running nginx-plus with app protect as ingress in a daemonset.
...
Any shells script commands copied to ``~/.local/bin`` wil be made executable and available to invoke from anywhere automatically.
...
Confirm the K8s cluter is healthy. Look for green 'clean'.
...
.. attention::
   ...
   You can restore the lab to a clean state at any time with the ``start-over`` flag:
